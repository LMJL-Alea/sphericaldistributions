...
static void gauss_legendre(int n, double a, double b, std::vector<double> &x,
  ...
  static const double xs[8] = {
      ...
      0.281603550779258913230460501460, 0.095012509837637440185319335425};
  static const double ws[8] = {
      0.027152459411754094851780572456, 0.062253523938647892862843836994,
      0.095158511682492784809925107602, 0.124628971255533872052476282192,
...
void BinghamDistribution::SetMeanAxis(const ValueType &val) {
  if (!this->BelongsToSupport(val)) {
    std::string msg = "The mean axis parameter of the Bingham distribution "
                      "should be a unit vector but its norm is ";
    ...
    cpp11::stop("The mean axis parameter of the Bingham distribution should be "
                "of unit norm.");
  }
...
double BinghamDistribution::GetCumulative(const ValueType &x) {
  ...
  return ComputeCumulativeIntegral(theta_max, phi_max) /
         m_NormalizationConstant;
}

double BinghamDistribution::ComputeNormalizationConstant(int n_theta,
                                                         int n_phi) const {
...
double BinghamDistribution::ComputeCumulativeIntegral(double theta_max,
                                                      double phi_max,
                                                      int n_theta,
                                                      int n_phi) const {
  std::vector<double> theta, w_theta;
 ...
 *
 * \param sample A numeric matrix of shape n x 3 specifying a sample of size n
 * drawn from the Bingham distribution on the 2-sphere.
 * \param method A string specifying the estimation method. Unused here.
 */
void BinghamDistribution::Fit(const SampleType &sample,
...
double BinghamDistribution::GetDistance(Self *otherDistribution) {
  ...
  SampleType thisBinghamSample(numberOfMonteCarloSamples, 3),
      otherBinghamSample(numberOfMonteCarloSamples, 3);
  GeneratorType generator;

  this->Random(thisBinghamSample, generator);
  BinghamDistribution *binghamDistr =
      dynamic_cast<BinghamDistribution *>(otherDistribution);
...
void BinghamDistribution::UpdateNormalizationConstant() {
  ...
  m_NormalizationConstant =
      8.0 * M_PI * std::exp(m_ConcentrationParameters.maxCoeff());
}
