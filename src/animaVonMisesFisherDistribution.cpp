...
void VonMisesFisherDistribution::SetMeanDirection(const ValueType &val) {
  if (!this->BelongsToSupport(val)) {
    std::string msg = "The mean direction parameter of the von Mises Fisher "
                      "distribution should be a unit vector but its norm is ";
    ...
    cpp11::stop("The mean direction parameter of the von Mises Fisher "
                "distribution should be of unit norm.");
  }
  ...
  m_NorthToMeanAxisRotationMatrix =
      anima::GetRotationMatrixFromVectors(m_MeanDirection, val);
  m_MeanDirection = val;
}

void VonMisesFisherDistribution::SetConcentrationParameter(const double &val) {
  if (val < 0)
    cpp11::stop("The concentration parameter of the von Mises Fisher "
                "distribution should be non-negative.");
  m_ConcentrationParameter = val;
  m_BesselRatio = anima::bessel_ratio_i_lower_bound(
      val, static_cast<double>(m_AmbientDimension) / 2.0);
}
...
double VonMisesFisherDistribution::GetLogDensity(const ValueType &x) {
  if (!this->BelongsToSupport(x))
    cpp11::stop("The log-density of the von Mises Fisher distribution is not "
                "defined for arguments outside the 2-sphere.");
...
double VonMisesFisherDistribution::GetCumulative(const ValueType &x) {
  ...
  double thetaCumul =
      (1.0 - std::exp(-m_ConcentrationParameter * (1.0 - std::cos(thetaVal))));
  thetaCumul /= (1.0 - std::exp(-2.0 * m_ConcentrationParameter));

  return phiCumul * thetaCumul;
}

void VonMisesFisherDistribution::GetQuantile(double p1, double p2,
...
void VonMisesFisherDistribution::Fit(const SampleType &sample,
   ...
   *for the mean direction and concentration parameter of the von Mises Fisher
   *distribution using the procedure described in Sra, A short note on parameter
   *approximation for von Mises-Fisher distributions: and a fast implementation
   *of Is(x), Computational Statistics, 2011.
   *
   * \author	Aymeric Stamm
   * \date	October 2023
   *
   * \param	sample A numeric matrix of shape n x 3 specifying a sample of
   *size n drawn from the von Mises Fisher distribution on the 2-sphere.
  ...
  double concentrationParameter =
      resultantValue * (ambientDimension - resultantValue * resultantValue);
  ...
  while (continueLoop) {
    ...
    double besselRatio = anima::bessel_ratio_i_lower_bound(
        concentrationParameter, ambientDimension / 2.0);
    double tmpValue = besselRatio - resultantValue;
    tmpValue /=
        (1.0 - besselRatio * besselRatio -
         (ambientDimension - 1.0) / concentrationParameter * besselRatio);
    concentrationParameter -= tmpValue;
    if (std::abs(concentrationParameter - oldConcentrationParameter) <
        this->GetEpsilon())
      continueLoop = false;
  }
...
void VonMisesFisherDistribution::Random(SampleType &sample,
...
double VonMisesFisherDistribution::GetDistance(Self *otherDistribution) {
  /**
   * \fn double
   * VonMisesFisherDistribution::GetDistance(VonMisesFisherDistribution
   * *otherDistribution)
   *
   * \author Aymeric Stamm
   * \date November 2023
   ...
   *
   * \return A numeric value storing the symmetric Kullback-Leibler divergence
   * to the input von Mises Fisher distribution. This is achieved following
   * Kitagawa & Rowley (2022), von Mises-Fisher distributions and their
   * statistical divergence, arXiv:2202.05192v1
   * (https://arxiv.org/pdf/2202.05192v1.pdf).
   */

  VonMisesFisherDistribution *vmfDistr =
      dynamic_cast<VonMisesFisherDistribution *>(otherDistribution);
  ...
  for (unsigned int i = 0; i < m_AmbientDimension; ++i)
    thisToOtherDist += (m_ConcentrationParameter * m_MeanDirection[i] -
                        otherConcentrationParameter * otherMeanDirection[i]) *
                       m_MeanDirection[i];
  thisToOtherDist *= m_BesselRatio;

  double otherToThisDist = 0.0;
  for (unsigned int i = 0; i < m_AmbientDimension; ++i)
    otherToThisDist += (otherConcentrationParameter * otherMeanDirection[i] -
                        m_ConcentrationParameter * m_MeanDirection[i]) *
                       otherMeanDirection[i];
...
RotationMatrixType VonMisesFisherDistribution::GetCovarianceMatrix() {
   ...
   * storing the covariance matrix of the von Mises Fisher distribution. This is
   * achieved following Kitagawa & Rowley (2022), von Mises-Fisher distributions
   * and their statistical divergence, arXiv:2202.05192v1
   * (https://arxiv.org/pdf/2202.05192v1.pdf).
   */

  RotationMatrixType covarianceMatrix;
  double diagConstant = m_BesselRatio / m_ConcentrationParameter;
  double offDiagConstant = 1.0 -
                           static_cast<double>(m_AmbientDimension) *
                               m_BesselRatio / m_ConcentrationParameter -
                           m_BesselRatio * m_BesselRatio;

  for (unsigned int i = 0; i < m_AmbientDimension; ++i) {
    covarianceMatrix(i, i) = diagConstant + offDiagConstant *
                                                m_MeanDirection[i] *
                                                m_MeanDirection[i];

    for (unsigned int j = i + 1; j < m_AmbientDimension; ++j) {
      double tmpValue =
          offDiagConstant * m_MeanDirection[i] * m_MeanDirection[j];
...
void VonMisesFisherDistribution::SampleFromVMFDistribution(
    ValueType &resVec, GeneratorType &generator) {
  /**
   * \fn void VonMisesFisherDistribution::SampleFromVMFDistribution(arma::vec3
   * &resVec, &generator)
   *
   * \brief Sample from the VMF distribution following Ulrich, G. (1984).
   * Computer generation of distributions on the m‐sphere. Journal of the Royal
   * Statistical Society: Series C (Applied Statistics), 33(2), 158-163.
   *
   * \author Aymeric Stamm
   * \date October 2013
   *
   * \param resVec An object of type arma::vec3 that will store a value sampled
   * from the VMF distribution.
   * \param generator An object of type std::mt19937 specifying which random
   * number generator to use.
   */

  double tmpVal =
      std::sqrt(m_ConcentrationParameter * m_ConcentrationParameter + 1.0);
  ...
  while (2.0 * std::log(T) - T + d < std::log(U)) {
    double Z = boost::math::quantile(m_BetaDistribution,
                                     m_RealUniformDistribution(generator));
...
void VonMisesFisherDistribution::SampleFromVMFDistributionNumericallyStable(
    ValueType &resVec, GeneratorType &generator) {
  /**
   * \fn void
   * VonMisesFisherDistribution::SampleFromVMFDistributionNumericallyStable(arma::vec3
   * &resVec, &generator)
   *
   * \brief Sample from the VMF distribution following Jakob, W. (2012).
   * Numerically stable sampling of the von Mises-Fisher distribution on Sˆ2
   * (and other tricks). Interactive Geometry Lab, ETH Zürich, Tech. Rep, 6.
   *
   * \author Aymeric Stamm
   * \date October 2013
   *
  ...
  double W =
      ...
      (std::log(xi) +
       std::log(1.0 -
                (xi - 1.0) * std::exp(-2.0 * m_ConcentrationParameter) / xi)) /
