...
double KummerMethod1(const double &x, const double &a, const double &b,
...
double KummerMethod2(const double &x, const double &a, const double &b,
                     const unsigned int maxIter, const double tol)
{
  double resVal = 1.0;
...
double KummerIntegrandMethod(const double &x, const double &a, const double &b)
{
  ...
  auto f1 = [&integrand](double t)
  { return integrand(t); };

  double resVal = boost::math::quadrature::gauss_kronrod<double, 61>::integrate(f1, 0.0, 1.0);

  double logMultConst = std::lgamma(b) - std::lgamma(a) - std::lgamma(b - a);
  resVal *= std::exp(logMultConst);

  return resVal;
}

double GetKummerFunctionValue(const double &x, const double &a, const double &b,
                              const unsigned int maxIter, const double tol)
{
  if ((a == 0) || (x == 0))
    return 1.0;
...
double GetScaledKummerFunctionValue(const double &x, const double &a, const double &b,
                                    const unsigned int maxIter, const double tol)
{
  if ((a == 0) || (x == 0))
    return std::exp(-x);
